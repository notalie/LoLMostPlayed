...
<html lang="en">
	<head>
		<meta charset="utf-8">
		<title>League - Most played champion</title>
...
/*$list = array();
	...
	$list[] = array(5 =>"e", 6 =>"f", 7 =>"g", 8 =>"h");
	
$i = 0;
foreach ($list as $key => $number) {
	$i++;
	foreach($number as $key => $letter){
		if ($i >= 2) break;
		echo $letter . "\n";
	}
}   */
...
//https://oc1.api.riotgames.com/lol/match/v4/timelines/by-match/230660886?api_key=RGAPI-c8ba1a9c-039d-413a-99b0-e35249784475
//https://oc1.api.riotgames.com/lol/summoner/v4/summoners/by-name/darkfeathered?api_key=RGAPI-16fa098f-d809-4dd2-af84-01cdeab5e1e1
$api_url = "https://oc1.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/itmSCmsngXyGjw0B4PsyynQGLvW0fc-k6Fmlyp_qex26tg?api_key=RGAPI-16fa098f-d809-4dd2-af84-01cdeab5e1e1";

$cache_filename = "cache/lol-cache.json";
...
if(is_array($data)) {
	...
	foreach ($data as $key => $title) {
		...
		foreach($title as $key => $description){
				if ($i > count($data)-1) break;

			if($key == "championId" && $i==1 ) {
				$key = "Most played Champ";
				$description = ChIDToName($description);
				echo"<section>".
					"<h1>".$key."</h1>".
					"<li>".$description."</li>".
				"</section>";
